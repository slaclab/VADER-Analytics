...
class Forward_MLPF:
    ...
    def __init__(self, numbus):
        ...
        self.n = 0
        self.test_y_vals = np.zeros((1,1))
        self.test_error_vals = np.zeros((1,1))
    ...
    def supply_full_data(self, p, q, v, a, n, N):
        ...
        uw = np.zeros((N,2*n))
        uw[:,np.arange(0,n)] = u
        uw[:,np.arange(n,2*n)] = w
        pq = np.zeros((N,2*n))
        pq[:,np.arange(0,n)] = p
        pq[:,np.arange(n,2*n)] = q
    ...
    def train_test_split_data(self, rand_percent = True, train_percent = 0.8, time_sers = False, time_percent = 0.8):
        ...
        if rand_percent:
            X_train, X_test, y_train, y_test = train_test_split(self.uw, self.pq, train_size=train_percent, random_state=42)
            Ntrain = np.shape(X_train)[0]
            Ntest = np.shape(X_test)[0]
        if time_sers:
            Ntrain = int(time_percent*np.shape(self.uw)[0])
            Ntest = np.shape(self.uw)[0] - Ntrain
            X_train = self.uw[np.arange(0,Ntrain),:]
            X_test = self.uw[np.arange(Ntrain,Ntrain+Ntest),:]
            y_train = self.pq[np.arange(0,Ntrain),:]
            y_test = self.pq[np.arange(Ntrain,Ntrain+Ntest),:]
    ...
    def test_error_svr(self):
        ...
        for i in range(self.Ntest):
            ...
            test_y_vals[i,:] = self.apply_svr(self.X_test[i,:])
